...
export const getJournals = async (req, res) => {
    try {
        ...
        const journals = await DailyJournal.find(filter)
...
export const updateJournal = async (req, res) => {
    ...
    try {
        const journal = await DailyJournal.findByIdAndUpdate(id, req.body, {
            new: true,
            runValidators: true,
        });
        if (!journal)
...
export const getAllEntriesForUser = async (req, res) => {
  try {
    const { userId } = req.body;

    const entries = await DailyJournal.find({ user: userId }).sort({ startTime: -1 });
